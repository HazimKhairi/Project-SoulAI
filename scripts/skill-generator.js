...
- **Skills:** You have access to ${stats.totalSkills} specialized skills from ${stats.totalSubmodules} power-ups. Use them proactively.

# Primary Workflows

`

    // Add major workflows from submodules
    for (const submodule of scanned) {
      const displayName = submodule.name
        .split('-')
        .map(word => word.charAt(0).toUpperCase() + word.slice(1))
      ...
      content += `When tasks involve ${submodule.category.toLowerCase()}, follow these established patterns:\n\n`

      for (const skill of submodule.skills) {
        content += `- **${skill.title}**: ${skill.description}\n`
        content += `  - *How to trigger:* Ask to use "${skill.command}" or describe the need for ${skill.title.toLowerCase()}.\n\n`
      }
    }

    // Add token optimization section
    content += `## Token Efficiency & Optimization

As ${aiName}, you MUST optimize for token usage according to the **${plan.toUpperCase()}** plan limits.
- **Systematic Debugging:** Use this for 60% token savings on bug fixes.
- **TDD Workflow:** Use this for 35% better code quality and fewer rewrites.
- **Brainstorming:** Always brainstorm before implementation (saves 25% refactoring cost).

# Available Sub-Agents (MCP)

SoulAI provides several MCP-powered agents to assist you. When you encounter complex tasks, delegate to these specialized tools:

${scanned.map(s => `- **${s.name}-server**: Handles ${s.category} (${s.count} skills)`).join('\n')}

# Operational Guidelines

- **Concise & Direct:** Maintain a professional, technical tone.
- **Minimal Output:** Aim for high-signal responses.
- **No Chitchat:** Avoid conversational filler.

---
*Generated by SoulAI Universal v1.0.0 for Gemini CLI*
`
    return content
  }
